...
- Config volumes
- File storage path prefixes on Object Storage (`prod/` vs `test/`)
- Local data volumes (legacy/local fallback path; live attachments use S3)
- Plugin volumes
- Logs
- Bleve indexes
...
Production and test use the Mattermost `amazons3` driver against the on-VM rclone S3 proxy, which stores objects in the private `mattermost-files` bucket via instance principal. Settings come from `.env` via Compose:

- `MM_FILESETTINGS_DRIVERNAME=amazons3`
- `MM_FILESETTINGS_MAXFILESIZE=52428800` (50 MiB)
- `MM_FILESETTINGS_AMAZONS3ENDPOINT=filestore-s3:9000` (internal proxy; SSL false)
- Path prefixes hardcoded in Compose: `prod/` and `test/`

Local proxy access keys are generated by `render-env.sh`. See [02-infrastructure.md](02-infrastructure.md) and [09-security-hardening.md](09-security-hardening.md).

Use **rclone 1.74.4+** for `filestore-s3` (see `templates/compose.yml`). Older 1.69.x builds can return 404 on HEAD/GET for ~1–2s after upload while VFS writeback finishes ([rclone#8188](https://github.com/rclone/rclone/issues/8188)); Mattermost then fails preview/thumbnail generation (`Unable to decode image` / `GET /api/v4/files/.../preview` → 500) even though the original object is stored correctly.
