...
Ubuntu security package updates are handled by `unattended-upgrades` with daily package-list updates and automatic unattended upgrades enabled.

## Backup Security

Keep the Object Storage bucket private and use instance principals instead of user API keys on the VM.

Recommended OCI bucket settings:

- Private visibility only.
- No public pre-authenticated requests unless deliberately created for a restore.
- Lifecycle rule to expire `daily/` objects after your retention window, for example 30 or 60 days.
